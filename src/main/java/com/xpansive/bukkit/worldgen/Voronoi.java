...
public class Voronoi {
	...
	private double distance(location a, location b) {
		...
		double d = (x * x) + (y * y); // Linear Squared (needs special care of
										// gain) try .05f
		// double d = Math.abs(x)+Math.abs(y); //Manhattan
		// if (Math.abs(x) == Math.abs(y) || Math.abs(x) < Math.abs(y)){return
		// Math.abs(x);} if (Math.abs(x) > Math.abs(y)){return Math.abs(y);} //
		// chebyshev
		// double d = ((x*x) + (x*y) + (y*y)); //quadratic try .08f for gain
		// float m = 15; //1 = manhattan 2 = linear squared 3 = more wiggly #2 4
		// =
		// double d = Math.pow((Math.pow(Math.abs(x),m) +
		// Math.pow(Math.abs(y),m)), (1 / m));
		return d;
	}

	private double check(int x, int y, double c, boolean f, int octave,
			int totaloctave) {
		double distance = c;
		// find closest point
		x = Math.max(x, 0);
		...
		for (int i = 0; i < (octave * g.a.size() / totaloctave); i++) {
			if (i == 0 && f) {
				...
				// distance =Math.min(distance(new
				// location(x,y),g.a.get(i)),distance);
				distance = Math.min(distance(new location(x, y), g.a.get(i)),
						distance);
			}
		}
		return distance;
	...
	double get(int xin, int yin, int octaves, float gain) {
		...
		for (int i = 1; i < octaves + 1; i++) {
			result += (((check(x, y, distance, true, i, octaves) / (size / gain)) * 2) - 1)
					/ octaves;
			gain += gain / octaves;
		}
...
// NOTES
// void voronoi(float3 position, out float f1, out float3 pos1, out float f2,
// out float3 pos2, float jitter=.9, bool manhattanDistance = false )
// {
