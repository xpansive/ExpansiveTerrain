		...
		if (random.nextInt(100) < 20) {
			...
			for (int y = start - 1; y != start; y %= 127, y = y < 0 ? 127 : y, y--) {
				...
				if (b.getTypeId() == 0
						...
						&& getLightLevel(world, x, y, z) < 12) {

					int cx = x, cz = z;

					Material mushroomType = random.nextBoolean() ? Material.BROWN_MUSHROOM
					...
					for (int i = 0; i < NUM_STEPS; i++) {
						...
						if (b.getTypeId() == 0
								&& world.getBlockTypeIdAt(cx, y - 1, cz) != 0
								&& getLightLevel(world, cx, y, cz) < 12) {
